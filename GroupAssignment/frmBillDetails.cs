...
namespace GroupAssignment
{
    public partial class frmBillDetails : Form
    {
        ...
        private bool CheckFormCalculation0()
        {
            ...
            try
            {
                if (!(float.TryParse(txtDiscount.Text, out float discount) && (discount >= 0 && discount < 100)))
                {
                    txtDiscount.Focus();
                    check = false;
                    MessageBox.Show("Sorry, discount must be in [0-100)% please!", "Message", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    btnSave.Enabled = false;
                }
        ...
        private decimal GrandTotal(float discount) => (decimal)(SubTotal() * (1 - (discount/100)));
